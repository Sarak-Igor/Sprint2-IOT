...
from backend.apps.asset_manager.infrastructure.models import (
    TelemetryMappingDB, ActiveAssetDB, MotorModelDB, DataVariableDB, SensorHardwareDB, TelemetryReadingDB
)
from backend.apps.asset_manager.domain.entities import (
    MotorModel, DataVariable, SensorHardware
)
        ...
        async with AsyncSessionLocal() as session:
            ...
            for asset in assets:
                formatted.append({
                    ...
                    "status": "Ativo" if asset.status == "operational" else "Alerta",
                    "health": "98%"
                })
            return formatted
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            stmt = (
                ...
                .join(DataVariableDB, TelemetryReadingDB.variable_id == DataVariableDB.id)
            ...
            for row in rows:
                ...
                result_asset = await session.execute(select(ActiveAssetDB).where(ActiveAssetDB.name == row.asset_name))
                ...
                if asset and asset.applied_thresholds:
                    ...
                    var_res = await session.execute(select(DataVariableDB).where(DataVariableDB.name == row.variable_name))
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            result = await session.execute(
                select(TelemetryReadingDB).order_by(TelemetryReadingDB.timestamp.desc()).limit(1)
            )
...
async def trigger_telegram_alert(payload: dict):
    ...
    if not token or not chat_id:
        return JSONResponse(status_code=500, content={"status": "error", "message": "Telegram Token ou Chat ID não configurados"})
    ...
    status_text = "ATENÇÃO DETECTADA" if status.upper() == "ATENÇÃO" else "PERIGO DETECTADO"
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=tg_payload) as response:
                if response.status != 200:
                    return JSONResponse(status_code=500, content={"status": "error", "message": "Falha na API do Telegram"})
                return {"status": "success", "message": "Alerta disparado pelo Gêmeo Digital"}
    except Exception as e:
...
def on_mqtt_message(client, userdata, msg):
    try:
        ...
        if main_loop and main_loop.is_running():
            asyncio.run_coroutine_threadsafe(process_and_broadcast(msg.topic, payload), main_loop)
...
async def websocket_telemetry(websocket: WebSocket):
    ...
    print(f"[DEBUG] WS: Novo cliente conectado. Total: {len(manager.active_connections)}")
    try:
        while True:
        ...
        print(f"[DEBUG] WS: Cliente desconectado. Total: {len(manager.active_connections)}")
    except Exception as e:
