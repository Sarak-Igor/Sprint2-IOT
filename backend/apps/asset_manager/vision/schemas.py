...
class PlateExtractionResult(BaseModel):
    """Saída estruturada exigida do LLM multimodal — nunca retornada ao cliente sem passar
    por esta validação. Campos como string (não numéricos) porque uma placa real mistura
    formatos (ex.: tensão tripla '220/380/440V', carcaça '132S')."""
    ...
    ip: str = Field(..., min_length=1, max_length=10)
